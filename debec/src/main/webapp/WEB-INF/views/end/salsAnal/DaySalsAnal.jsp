...
<html>
	<head>
		...
		<!-- 원 변환 1,000 -->
		<script src="resources/js/wonPointer.js"></script>	
	</head>
	<body>
		...
		<div class="container">
			...
			<div style="display:inline-block; margin-top:1%; float:right">
				<h5>
					...
					<a href="/DaySalsAnal.do" style="text-decoration:none"><strong>일별 매출 분석</strong></a>
				</h5>
			</div>	
		</div>
			...
			<input type="hidden" name="btnDatSech" value="${SalsValueObject.btnDatSech}"> <!-- 버튼 일자 선택-->
				<table class="table table-bordered">
					<tr>
						...
						<td>
							<input type="button" class="btn btn-default" value="3일" onclick="javascirpt:DaySalsAnal('3day')">
							<input type="button" class="btn btn-default" value="7일" onclick="javascirpt:DaySalsAnal('7day')">
							<input type="button" class="btn btn-default" value="14일" onclick="javascirpt:DaySalsAnal('14day')">
					...
					<tr>
						...
						<td>
							<select name="topCatgrNme" id="topCatgrNme" class="form-control" onchange="javascript:catgrChoice('대',this.value)">
							...
							<select name="midCatgrNme" id="midCatgrNme" class="form-control" onchange="javascript:catgrChoice('중',this.value)">
		...
		<div class="container text-center">
			<input type="button" class="btn btn-default" value="검색" onclick="javascript:salsSech()">
		</div>
		...
		<script>
			...
			function drawMultSeries() {
				...
				data.addColumn('string', '일자'); // DataTable 객체에서 컬럼 값을 정의 해준다.Column은 그래프가 사용 될 데이터 순서를 지정해준다.(데이터 형식과 명칭을 정해줌.)
			...
			function xlsDownload(){
				document.xlsDownFrm.action = "/daySalsXls.do";
				document.xlsDownFrm.submit();
			};
			
			// 날짜 버튼눌렀을 때 (1,3,7,14일)
	   		function DaySalsAnal(day) {
	   			var url = "/DaySalsAnal.do?btnDatSech="+day;
	   		...
	   		function salsSech(){	
	            ...
	            var startDate = $("#datepicker1").val(); // 시작날짜
	            var startDateArr = startDate.split('-');   
	            var endDate = $("#datepicker2").val(); // 종료날짜
	            ...
	            var day = 1000*60*60*24;                 
	           	if(startDate=="" && endDate == ""){           		
	   	             ...
	   	             else if(parseInt((endDateCompare-startDateCompare)/day) > 30){
	   	                 alert("일별 매출에서는 최대 30일까지 분석이 가능합니다.");
	   	                 return;
	   	             }
	   			...
	   			document.salsSechFrm.action ="/DaySalsAnal.do";
